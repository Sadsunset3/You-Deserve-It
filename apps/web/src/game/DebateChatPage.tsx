...
export function DebateChatPage({ room, send }: Props) {
  ...
  const composerDisabled = room.phase !== 'debate-chat' || pending || !room.me.connected || room.opponent?.connected === false;
  ...
  const phaseText = room.phase === 'round-adjudicating' ? '列车长正在整理整轮交锋并裁决' : room.phase === 'round-result' ? '本轮已经封口，下一轮即将开始' : '双方可自由发言，发言代表你自己和所在轨道';

  return <section className={`debate-chat-page phase-${room.phase}`} aria-label="攻防聊天室">
    <header className="debate-chat-header">
      <div><span>审判室 {room.roomCode}</span><strong>第 {room.round}/3 轮 · {isAttacker ? '你是攻方' : '你是守方'}</strong></div>
      ...
      <section className="debate-transcript-panel">
        ...
        <div className="debate-transcript" ref={transcriptRef} role="log" aria-live="polite">
          ...
          {messages.map((message) => {
            ...
            return <article className={`transcript-entry seat-${message.sender}`} data-testid="debate-message" key={`${message.sender}-${message.messageId}`}><div><span>{message.sender === 'a' ? '甲' : '乙'}</span><strong>{nickname}</strong><time dateTime={message.sentAt}>{new Date(message.sentAt).toLocaleTimeString('zh-CN', { hour: '2-digit', minute: '2-digit', second: '2-digit' })}</time></div><p>{message.text}</p></article>;
          })}
          {room.roundVerdict && <article className="conductor-verdict-entry" data-testid="conductor-verdict"><ConductorAvatar name={room.conductor!.name}/><div><strong>{room.roundVerdict.winnerSeat === 'a' ? '甲方胜出' : '乙方胜出'}</strong><p>{room.roundVerdict.conductorMessage}</p></div></article>}
        ...
        <div className="debate-composer">
          ...
          <textarea id="debate-message-input" aria-label="发送辩论消息" maxLength={2000} disabled={composerDisabled} value={text} onChange={(event) => setText(event.target.value)} onKeyDown={(event) => { if (event.key === 'Enter' && !event.shiftKey) { event.preventDefault(); void sendMessage(); } }}/>
