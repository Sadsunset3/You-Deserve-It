...
const verdict = await createAiGateway({ apiKey }).decideRound({
  ...
  attacker: 'a',
  defender: 'b',
  players: { a: { nickname: '验证甲方' }, b: { nickname: '验证乙方' } },
  ...
  messages: [
    { messageId: 'verify-a', sequence: 1, sender: 'a', text: '该人物曾经欺骗无辜者，造成了实际损失。', sentAt: new Date(startedAt).toISOString() },
    { messageId: 'verify-b', sequence: 2, sender: 'b', text: '该人物后来主动赔偿，并协助避免了更大的损失。', sentAt: new Date(startedAt + 1).toISOString() },
  ],
  priorRounds: [],
});

const summary = { model: config.model, elapsedMs: Date.now() - startedAt, winnerSeat: verdict.winnerSeat, fallback: verdict.fallback };
